...
function Sidebar({ onNavigate, currentUser }) {
  ...
  const getNavItemsForRole = (userRole) => {
    const baseItems = {
      ...
      attendance: { id: 'attendance', label: 'Attendance', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
        </svg>
      )},
      'leave-requests': { id: 'leave-requests', label: 'Leave Requests', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
      )},
      'missing-items': { id: 'missing-items', label: 'Missing Items', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      'my-attendance': { id: 'my-attendance', label: 'My Attendance', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    switch (userRole) {
      case 'Student':
        return [
          baseItems.dashboard,
          baseItems['my-attendance'],
          baseItems['leave-requests'],
          baseItems['missing-items']
        ];
      case 'Teacher':
        return [
          baseItems.dashboard,
          baseItems.students,
          baseItems.attendance,
          baseItems['leave-requests'],
          baseItems['missing-items'],
          baseItems.settings
        ];
      default: // Administrator, Warden, Staff
        return [
          baseItems.dashboard,
          baseItems.students,
          baseItems.attendance,
  ...
  const navItems = getNavItemsForRole(currentUser?.role || 'Administrator');
